create database umang_lab8_stud_enrol;
use umang_lab8_stud_enrol;
...
select c.cname from class c  where c.room = 301
or c.cname in (select e.cname from enrolled e group by e.cname having count(*) >= 5);


select distinct sname  from student s where s.snum in (select e1.snum
   from enrolled e1,enrolled e2,class c1,class c2 where e1.snum=e2.snum and e1.cname=c1.cname and e2.cname=c2.cname and c1.meets_at='11:15:02');
