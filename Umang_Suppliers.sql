REATE TABLE supplier(sid int, sname char(30), address char(30), PRIMARY KEY(sid));
...
-- 2
SELECT S.sname FROM supplier S WHERE NOT EXISTS ((SELECT P.pid FROM parts P) EXCEPT(SELECT C.pid FROM catalog C WHERE C.sid = S.sid ));
-- 3
SELECT S.sname FROM supplier S WHERE NOT EXISTS((SELECT P.pid FROM parts P WHERE P.color='red') EXCEPT (SELECT C.pid FROM catalog C, parts P WHERE C.sid = S.sid AND C.pid = P.pid AND P.color = 'red'));
-- 4
